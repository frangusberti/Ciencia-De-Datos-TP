...
- Para lograr una visión completa y eficiente, se programó un Dataframe resumen automatizado que agrupa y expone por cada columna: el tipo de dato subyacente, la cantidad de valores únicos (`nunique()`), el conteo total de valores nulos y su porcentaje relativo sobre la población total.
- Este reporte dinámico expuso la presencia de múltiples valores nulos en columnas clave, destacándose faltantes en ingresos (`Income`), cantidad de hijos (`Kidhome`, `Teenhome`), número de quejas (`Complain`) y respuestas a campañas promocionales.
...
- **Filtros lógicos directos:** A diferencia de los atípicos estadísticos, se aplicó un filtro directo de eliminación para los errores irrefutables de sistema o extremos excepcionales. Mediante una máscara booleana se filtraron y eliminaron todos los registros donde `Year_Birth < 1930` (edades biológicamente improbables), `Income < 0` (ingresos negativos) e `Income > 150000` (sueldos atípicamente altos que desvirtuarían la muestra).
- **Reducción de dimensionalidad programática:** En lugar de eliminar variables "a mano", se desarrolló una lógica dinámica iterando sobre todas las columnas del dataset y comprobando si poseían un único valor constante (`nunique() == 1`). Este filtro automático detectó a `Z_CostContact` y `Z_Revenue` (con valores únicos de 3 y 11 respectivamente) y las eliminó en masa mediante `drop`, ya que la falta de varianza impide que aporten información analítica.
